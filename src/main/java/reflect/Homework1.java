...
public class Homework1 {
    ...
    public void streamPipeline2() {
        Arrays.stream(String.class.getDeclaredMethods()).map(method -> method.getName()).distinct().sorted().forEach(System.out::println);
    }

    /**
     * Prints the declared methods of java.lang.String with two or more parameters whose parameters are all of the
    ...
    public String streamPipeline7() {
        return Arrays.stream(String.class.getDeclaredMethods()).map(method -> method.getName()).max(Comparator.comparingInt(method -> method.length())).get();
    }
    ...
    public String streamPipeline8() {
        return Arrays.stream(String.class.getDeclaredMethods()).filter(method -> Modifier.isPublic(method.getModifiers())).map(method -> method.getName()).max(Comparator.comparingInt(method -> method.length())).get();
    }
    ...
    public Method streamPipeline11() {
        return Arrays.stream(String.class.getDeclaredMethods()).max(Comparator.comparingInt(method -> method.getParameterCount())).get();
    }
